...
void MainWindow::mousePressEvent(QMouseEvent *event)
{
        ...
        *mainPath = new_solution;       // After clipping: clipped polygon = main polygon (current polygon)
        ...
        for(unsigned int i = 0; i < new_solution.size(); i++)
        {
            ...
            for(unsigned int j = 0; j < new_solution[i].size(); j++)
            {
                ...
                myPolygonBodyVertices[i].x = toB2((int)new_solution[i][j].X);
                myPolygonBodyVertices[i].y = toB2((int)new_solution[i][j].Y);
            }

            /*myPolygonBodies.append(new PolygonBody(myPolygonBodyVertices, new_solution[i].size(),
                                                   b2Vec2(0, 0), world));*/
    ...
    if(mouseRightKeyPressed  && !busy) {
        scene->addItem(new Circle(toB2(10), QPointF(toB2(event->pos().x()), toB2(event->pos().y())), world));
